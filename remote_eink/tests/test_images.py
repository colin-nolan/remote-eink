...
from remote_eink.images import Image, ImageType, SimpleImage
...
class TestImage(unittest.TestCase):
    ...
    def test_get_data_with_cache(self):
        image = SimpleImage(_IDENTIFIER, lambda: BytesIO(_DATA).read(), _IMAGE_TYPE, cache_data=True)
    ...
    def test_get_data_without_cache(self):
        image = SimpleImage(_IDENTIFIER, lambda: _DATA, _IMAGE_TYPE, cache_data=False)
        self.assertEqual(_DATA, image.data)
        self.assertEqual(_DATA, image.data)
    ...
    def test_cache_toggle(self):
        for start_state in (True, False):
            with self.subTest(start_state=start_state):
                image = SimpleImage(_IDENTIFIER, lambda: _DATA, _IMAGE_TYPE, cache_data=start_state)
                self.assertEqual(_DATA, image.data)
